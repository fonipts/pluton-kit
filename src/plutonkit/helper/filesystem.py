...
def generate_default_file(reference_value,name,content):
    DIRECTORY = os.getcwd()
    f = open(os.path.join(DIRECTORY, default_project_name(reference_value['details']['project_name']),name), 'w')
    f.write(content)
    f.close()
...
def generate_requirement(reference_value,library):
    DIRECTORY = os.getcwd()
    f = open(os.path.join(DIRECTORY, default_project_name(reference_value['details']['project_name']),REQUIREMENT), 'w')
