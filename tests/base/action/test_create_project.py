...
class TestCreateProject(unittest.TestCase):
    ...
    @patch("plutonkit.command.action.create_project.input", side_effect=["proj", "y"])
    @patch("plutonkit.command.action.create_project.answer_yes", return_value=True)
    ...
    def test_project_details_execute_yes(self, mock_exit, mock_fb, mock_ans, mock_input):
        self.proj.project_details_execute("remote")
        mock_fb.assert_called_with("remote")
        mock_fb.return_value.set_folder_name.assert_called_with("proj")
        mock_fb.return_value.execute_create_project.assert_called()
        mock_exit.assert_called_with(0)

    @patch("plutonkit.command.action.create_project.input", side_effect=["proj", "n"])
    @patch("plutonkit.command.action.create_project.answer_yes", return_value=False)
    def test_project_details_execute_no(self, mock_ans, mock_input):
