"""Module providing a function printing python version."""
DOCKER_FILE:str = "Dockerfile"
DOCKER_COMPOSE_FILE:str = "docker-compose.yaml"
INTRODUCTION:str = "Welcome to pluton-kit, this is your application builder in python woorld"

PROJECT_COMMAND_FILE:str = "command.yaml"
PROJECT_DETAILS_FILE:str = "project.yaml"
ARCHITECTURE_DETAILS_FILE:str = "architecture.yaml"
REMOTE_URL_RAW:str = "https://raw.githubusercontent.com/fonipts/pluton-lobby/experiment_4_2024/blueprint"
PROJECT_ENV_FILE:str = ".env"
REQUIREMENT:str = "requirements.txt"
