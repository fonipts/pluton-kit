...
class SchemaCommand:
    ...
    def get_error(self) ->  List[str]:
        ...
        for key,value in self.reference_value.items():
            ...
            if max_distance != 1.0:
                list_error.append(f"the `{key}` is invalid,  I assume you are using this command instead `{VALID_KEY[distances.index(max_distance)]}`")
            if key == "script":
        ...
        for _,value1 in value.items():
            for key2,value2 in value1.items():
                ...
                if sub_max_distance != 1.0:
                    list_error.append(f"the `{key2}` is invalid,  I assume you are using this command instead `{COMMAND_VALID_KEY[sub_distances.index(sub_max_distance)]}`")
                if key2 == "group":
